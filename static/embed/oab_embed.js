...
_L.each = function(elems, key, val) {
    ...
    for (i = 0, len = elems.length; i < len; i++) {
      ...
      if (elem != null) {
        if (typeof key === 'function') {
          ...
          results.push(_L.set(elem, key, val));
        }
      } else {
        results.push(void 0);
      }
...
_L.listen = function(action, els, fn) {
  return _L.each(els, function(el) {
    var wfn;
    if (action === 'enter') {
      action = 'keyup';
      wfn = function(e) {
...
_L.show = function(els, html, append) {
  return _L.each(els, function(el) {
    var was;
...
_L.get = function(els, attr) {
  var res;
  res = void 0;
  _L.each(els, function(el) {
    if (attr == null) {
      try {
        res = el.value;
      } catch (error1) {}
...
_L.html = function(els, html, append, show) {
  var ref, rs;
  rs = [];
  _L.each(els, function(el) {
    if (typeof html === 'string') {
...
_L.append = function(els, html) {
  return _L.html(els, html, true);
};

_L.remove = function(els) {
  return _L.each(els, function(el) {
    return el.parentNode.removeChild(el);
  });
};

_L.class = function(el, cls) {
...
_L.classes = function(els) {
  return _L.class(els);
};

_L.has = function(el, cls) {
  var classes;
...
_L.css = function(els, key, val) {
  return _L.each(els, function(el) {
    var i, k, len, p, ps, ref, ref1, s, ss, style;
    s = (ref = _L.get(el, 'style')) != null ? ref : '';
    style = {};
    ref1 = s.split(';');
    ...
    return _L.set(el, 'style', ss);
  });
};

_L.jx = function(route, q, success, error, api, method, data, headers) {
  var h, p, url, xhr;
  ...
  if (route.startsWith('http')) {
    ...
    url = api + (api.endsWith('/') ? '' : '/');
    if (route) {
      url += (route.startsWith('/') ? route.replace('/', '') : route);
    }
  }
  if (typeof q === 'string') {
  ...
  xhr.open(method != null ? method : 'GET', url);
  for (h in headers != null ? headers : {}) {
  ...
  xhr.onload = function() {
    ...
    if (typeof _L.loaded === 'function') {
      return _L.loaded(xhr);
    }
  };
...
_L.post = function(route, data, success, error, api, headers) {
  if (headers == null) {
    headers = {};
  }
  if (typeof data === 'object') {
    if (typeof data.append !== 'function') { // a FormData object will have an append function, a normal json object will not. FormData should be POSTable by xhr as-is
  ...
  return _L.jx(route, void 0, success, error, api, 'POST', data, headers);
};
...
_oab = function(opts) {
  ...
  try {
    ...
    if (this.demo == null) {
      this.demo = window.location.href.indexOf('/demo') !== -1 && (window.location.href.indexOf('openaccessbutton.') !== -1 || window.location.href.indexOf('shareyourpaper.') !== -1 || window.location.href.indexOf('instantill.') !== -1);
    }
    if (this.loaded != null) {
      _L.loaded = this.loaded; // if this is set to a function, it will be passed to _leviathan loaded, which gets run after every ajax call completes. It is also called directly after every configure
    }
    if (window.location.search.indexOf('local=') !== -1) {
      this.local = window.location.search.indexOf('local=true') !== -1 ? true : false;
    }
    if (window.location.search.indexOf('clear=') !== -1 || this.local === false) {
      try {
    ...
    if (!this.config.autorun_off) {
      ...
      for (j = 0, len1 = ap.length; j < len1; j++) {
        ...
        if (window.location.search.replace('?', '&').indexOf('&' + o + '=') !== -1) {
          this.data[eq != null ? eq : o] = decodeURIComponent(window.location.search.replace('?', '&').split('&' + o + '=')[1].split('&')[0].replace(/\+/g, ' '));
        }
      }
      if (!this.data.doi && window.location.href.split('?')[0].indexOf('/10.') !== -1 && window.location.href.split('?')[0].split('/10.')[1].indexOf('/') > 1 && window.location.href.split('?')[0].split('/10.')[1].trim().split('/').length > 1) {
    ...
    if (window.location.search.indexOf('confirmed=') !== -1) {
...
_oab.prototype.loading = function(load) {
  ...
  if (load !== true && (this._loading || load === false)) {
    ...
    this._loading = false;
    return _L.each('._oab_loading', (el) => {
...
_oab.prototype.state = function(pop) {
  ...
  if (this.pushstate) {
    try {
      ...
      if (pop == null) {
        if (window.location.href.indexOf('shareyourpaper.org') !== -1) {
          if (window.location.href.indexOf('/10.') !== -1 || window.location.href.replace(/\//g, '').endsWith('.org')) {
            ...
            u += u.indexOf('?') === -1 ? '?' : '&';
            u += 'doi=' + this.data.doi;
            if (window.location.search.split('?doi=')[1].indexOf('&') !== -1) {
        ...
        } else if (window.location.href.indexOf('/setup') === -1 && window.location.href.indexOf('/demo') === -1) {
...
_oab.prototype.ping = function(what) {
  var url;
  try {
    if (what.indexOf(this.plugin) === -1) {
      if (!what.startsWith('_')) {
        what = '_' + what;
      }
      what = this.plugin + what;
    }
    url = this.api.indexOf('dev.') !== -1 ? 'https://dev.api.cottagelabs.com' : 'https://api.cottagelabs.com';
...
_oab.prototype.panel = function(panel, section) {
  var he;
  if (he = _L.gebi('_oab_' + (panel.startsWith('_oab_') ? panel.replace('_oab_', '') : panel))) {
    _L.hide('._oab_panel');
    _L.show(he);
    if (section) {
...
_oab.prototype.section = function(section) {
  ...
  fe = _L.gebi('_oab_' + (section.startsWith('_oab_') ? section.replace('_oab_', '') : section));
  if (fe == null) {
    fe = _L.gebc('_oab_' + (section.startsWith('_oab_') ? section.replace('_oab_', '') : section));
  }
  if (fe) {
...
_oab.prototype.submit = function(e) { // only used by instantill
  ...
  try {
    ...
    if (!this.openurl() && !this.data.email && _L.gebi('#_oab_email')) {
      ...
      for (i = 0, len = ref4.length; i < len; i++) {
        ...
        if (data.metadata.doi && data.metadata.doi.indexOf('http') === 0) {
      ...
      if (this.demo === true) {
        ...
        return _L.post(this.api + '/ill', data, (res) => {
...
_oab.prototype.validate = function() {
  var email, ref;
  if (this.config.terms && !_L.checked('#_oab_read_terms')) { // instantill terms
    return _L.show('#_oab_error', '<p>Please agree to the terms first.</p>');
  } else {
    email = ((ref = _L.get('#_oab_email')) != null ? ref : '').trim();
    if (!email.length) {
      _L.show('#_oab_error', '<p>Please provide your university email address.</p>');
      _L.css('#_oab_email', 'border-color', '#f04717');
      return _L.gebi('#_oab_email').focus();
    } else {
      this.loading();
      return _L.post(this.api + '/validate?uid=' + this.uid + '&email=' + email, this.config, (res) => {
        this.loading(false);
        if (res === true) {
          ...
          return _L.show('#_oab_error', '<p>Please try again with your university email address.</p>');
        } else {
          return _L.show('#_oab_error', '<p>Sorry, your email does not look right. ' + (res !== false ? 'Did you mean ' + res + '? ' : '') + 'Please check and try again.</p>');
        }
      }, () => {
        this.data.email = _L.get('#_oab_email').trim();
        if (this.plugin === 'instantill') {
...
_oab.prototype.metadata = function(submitafter) { // only used by instantill
  ...
  for (i = 0, len = ref.length; i < len; i++) {
    ...
    if ((((ref1 = this.f) != null ? (ref2 = ref1.metadata) != null ? ref2[m] : void 0 : void 0) != null) || (this.data[m] != null)) {
      _L.set('#_oab_' + m, ((ref3 = this.f.metadata) != null ? ref3 : this.data)[m]);
    }
  }
  _L.hide('._oab_panel');
  return _L.show('#_oab_metadata');
};
...
_oab.prototype.done = function(res, msg) {
  ...
  if (ou = this.openurl()) {
    ...
    _L.hide('._oab_panel');
    _L.hide('._oab_done');
    if (typeof res === 'string' && _L.gebi('_oab_' + res)) {
      ...
      _L.html('#_oab_done_header', '<h3>Thanks! Your request has been received</h3><p>Your confirmation code is: ' + res + ', this will not be emailed to you. The ' + (this.config.say_paper ? 'paper' : 'article') + ' will be sent to ' + this.data.email + ' as soon as possible.</p>'); // only instantill falls through to here
    } else {
...
_oab.prototype.deposit = function(e) { // only used by shareyourpaper
  ...
  try {
    ...
    if (!this.data.email && _L.gebi('#_oab_email')) {
      return this.validate();
    } else if (this.demo === true && (this.data.doi != null) && this.data.doi.indexOf('10.1234/oab-syp-') === 0) {
      if ((this.data.doi != null) && this.data.doi.indexOf('10.1234/oab-syp-') === 0 && this.data.doi !== '10.1234/oab-syp-confirm') { // demo successful deposit
      ...
      return _L.post(this.api + '/deposit', data, (res) => { // + (this.f.catalogue ? '')
        var ref10, ref11, ref12, ref13, ref3, ref4, ref5, ref6, ref7, ref8, ref9;
        this.loading(false);
        if (typeof this.file !== 'boolean') {
          if (((ref3 = res.zenodo) != null ? ref3.already : void 0) || (this.data.confirmed && !((ref4 = res.zenodo) != null ? ref4.url : void 0))) { //or not this.f?.permissions?.file?.archivable
            ...
            if (res.embargo) {
              info = '<p>You\'ve done your part for now. Unfortunately, ' + ((ref6 = (ref7 = (ref8 = this.f) != null ? (ref9 = ref8.metadata) != null ? ref9.shortname : void 0 : void 0) != null ? ref7 : (ref10 = this.f) != null ? (ref11 = ref10.metadata) != null ? ref11.journal : void 0 : void 0) != null ? ref6 : 'the journal') + ' won\'t let us make it public until ';
  ...
  try {
    ...
    if (!_L.gebi(this.element)) {
      ...
      if (((ref = this.f) != null ? ref.doi_not_in_crossref : void 0) && ((ref1 = this.f) != null ? ref1.doi_not_in_oadoi : void 0)) {
        ...
        return _L.gebi('_oab_input').focus();
        ...
        if (this.cml() || (this.config.old_way && this.config.old_way.includes('@'))) {
          ...
          nj += this.config.old_way ? (this.config.old_way.indexOf('@') !== -1 ? 'mailto:' : '') + this.config.old_way : 'mailto:' + this.cml();
        ...
        _L.html('._oab_your_paper', (((ref17 = this.f) != null ? (ref18 = ref17.permissions) != null ? (ref19 = ref18.best_permission) != null ? ref19.version : void 0 : void 0 : void 0) === 'publishedVersion' ? 'the publisher pdf of ' : '') + paper);
        _L.html('._oab_journal', (ref20 = (ref21 = this.f) != null ? (ref22 = ref21.metadata) != null ? ref22.shortname : void 0 : void 0) != null ? ref20 : 'the journal');
        if (this.f.url) {
          ...
          if (this.config.oa_deposit_off) {
            _L.hide('._oab_get_email');
            return _L.show('._oab_oa');
          } else {
            this.file = true; // no file required for oa deposit...
            return _L.show('._oab_oa_deposit');
          }
          ...
          if (((ref26 = this.f) != null ? (ref27 = ref26.permissions) != null ? (ref28 = ref27.best_permission) != null ? ref28.version : void 0 : void 0 : void 0) === 'publishedVersion') {
            ...
            _L.hide('#_oab_not_pdf');
          }
          if (typeof ((ref29 = this.f) != null ? (ref30 = ref29.permissions) != null ? (ref31 = ref30.best_permission) != null ? ref31.licence : void 0 : void 0 : void 0) === 'string' && this.f.permissions.best_permission.licence.indexOf('other-') === 0) {
            _L.html('._oab_licence', 'under the publisher\'s terms' + refs);
          } else {
            _L.html('._oab_licence', (ref32 = (ref33 = this.f) != null ? (ref34 = ref33.permissions) != null ? (ref35 = ref34.best_permission) != null ? ref35.licence : void 0 : void 0 : void 0) != null ? ref32 : 'CC-BY');
          }
          return _L.show('._oab_archivable');
          ...
          _L.hide('._oab_get_email');
          return _L.show('._oab_permission_required');
        } else {
          // can't be directly shared but can be passed to library for dark deposit
          this.file = true;
          _L.hide('#_oab_file');
          return _L.show('._oab_dark_deposit');
        }
      }
    }
  } catch (error1) {
      ...
      if ((((ref11 = this.f.metadata) != null ? ref11.title : void 0) != null) && ((this.f.metadata.journal != null) || this.data.usermetadata)) {
        ...
        if (((ref13 = this.f.ill) != null ? (ref14 = ref13.subscription) != null ? ref14.journal : void 0 : void 0) || ((ref15 = this.f.ill) != null ? (ref16 = ref15.subscription) != null ? ref16.url : void 0 : void 0)) {
          ...
          if (this.f.ill.subscription.url != null) {
            ...
            _L.set('#_oab_sub_url', 'href', this.f.ill.subscription.url);
          }
        ...
        if ((this.f.ill != null) && !((this.config.ill_if_sub_off && hassub) || (this.config.ill_if_oa_off && hasoa))) {
          _L.html('#_oab_cost_time', '<p>It ' + (this.config.cost ? 'costs ' + this.config.cost : 'is free to you,') + ' and we\'ll usually email the link within ' + ((ref17 = this.config.time) != null ? ref17 : '24 hours') + '.<br></p>');
          if (!this.data.email) {
            if (this.openurl()) {
          ...
          return _L.show('#_oab_ask_library');
        }
      } else if (this.data.usermetadata) {
        _L.html('#_oab_citation', '<h3>Unknown ' + (this.config.say_paper ? 'paper' : 'article') + '</h3><p>Sorry, we can\'t find this ' + (this.config.say_paper ? 'paper' : 'article') + ' or sufficient metadata. ' + this.contact() + '</p>');
...
_oab.prototype.find = function(e) {
  ...
  try {
    ...
    if (this.data.doi && this.data.doi.indexOf('10.') !== -1 && (this.data.doi.indexOf('/') === -1 || this.data.doi.indexOf('http') === 0)) {
    ...
    if (val = _L.get('#_oab_input')) {
      ...
      if (val.length) {
        if (val.indexOf(' ') === -1) {
          ...
          } else if (val.indexOf('10.') !== -1) {
            this.data.doi = val;
          } else if (val.indexOf('http') === 0 || val.indexOf('www.') !== -1) {
            this.data.url = val;
      ...
      _L.set('#_oab_input', (ref1 = (ref2 = (ref3 = this.data.doi) != null ? ref3 : this.data.title) != null ? ref2 : this.data.url) != null ? ref1 : this.data.id);
    }
    if (this.plugin === 'instantill' && !this.data.doi && !this.needmore && !((ref4 = this.f) != null ? (ref5 = ref4.metadata) != null ? ref5.journal : void 0 : void 0) && (!this.data.title || (this.data.title.length < 30 && this.data.title.split(' ').length < 3))) {
      ...
      if (this.plugin === 'shareyourpaper') {
        ...
        delete this.data.id;
        return _L.show('#_oab_error', '<p>Please provide a DOI. If you\'re not sure what a DOI is, go <a id="_oab_doi_howto" href="https://library.uic.edu/help/article/1966/what-is-a-doi-and-how-do-i-use-them-in-citations" target="_blank">here</a>.</p>');
      } else {
        return _L.show('#_oab_error', '<p><span>&#10060;</span> Sorry please provide the full DOI, title, citation, PMID or PMC ID.</p>');
      }
    } else if (this.data.doi && this.plugin === 'shareyourpaper' && (this.data.doi.indexOf('10') !== 0 || this.data.doi.indexOf('/') === -1 || this.data.doi.indexOf('.') === -1 || this.data.doi.length < 8)) {
      delete this.data.doi;
      _L.set('#_oab_input', '');
      _L.gebi('_oab_input').focus();
      return _L.show('#_oab_error', '<p>Please provide a DOI. If you\'re not sure what a DOI is, go <a id="_oab_doi_howto" href="https://library.uic.edu/help/article/1966/what-is-a-doi-and-how-do-i-use-them-in-citations" target="_blank">here</a>.</p>');
      ...
      if (this.demo === true && (this.data.title === 'Engineering a Powerfully Simple Interlibrary Loan Experience with InstantILL' || this.data.doi === '10.1234/567890' || ((this.data.doi != null) && this.data.doi.indexOf('10.1234/oab-syp') === 0))) {
        data = {
          metadata: {
        ...
        return _L.post(this.api + '/find', this.data, (data) => {
          if (this.plugin === 'instantill') {
            return this.findings(data);
          } else {
            return this.permissions(data);
          ...
          return _L.show('#_oab_error', '<p>Oh dear, the service is down! We\'re aware, and working to fix the problem. ' + this.contact() + '</p>');
        });
      }
    }
  } catch (error1) {
...
_oab.prototype.configure = function(key, val, build, preview) {
  ...
  if (((typeof key === 'string' && (val != null)) || ((key == null) && (val == null))) && JSON.stringify(this.config) === '{}') {
    ...
    if (this.remote !== false && this.uid && this.uid !== 'anonymous' && JSON.stringify(this.config) === '{}') { // should a remote call always be made to check for superseded config if one is not provided at startup?
      _L.jx(this.api + '/' + (this.plugin === 'instantill' ? 'ill' : 'deposit') + '/config?uid=' + this.uid, void 0, (res) => {
    ...
    if (_L.gebi(this.element)) {
      if (build !== false) {
        ...
        if (this.bootstrap == null) {
          ...
          this.bootstrap = dstr.indexOf('bootstrap/') !== -1 || dstr.indexOf('/bootstrap') !== -1 || dstr.indexOf('bootstrap.css') !== -1 || dstr.indexOf('bootstrap.min.css') !== -1 || dstr.indexOf('btn-') !== -1;
          if (this.bootstrap === true) {
            console.log('Found bootstrap indicators in the doc');
          }
        }
        if (this.bootstrap === true) {
          if (this.template.indexOf('btn-primary') === -1) {
            this.template = this.template.replace(/_oab_button/g, '_oab_button btn btn-primary').replace(/_oab_form/g, '_oab_form form-control');
          }
        } else if (this.template.indexOf('btn-primary') !== -1) {
        ...
        if (this.data.doi || this.data.title || this.data.url || this.data.id) {
          _L.set('#_oab_input', (ref = (ref1 = (ref2 = this.data.doi) != null ? ref2 : this.data.title) != null ? ref1 : this.data.url) != null ? ref : this.data.id);
        }
        _L.each('._oab_paper', (el) => {
          var cs;
          cs = el.innerHTML;
          if (this.config.say_paper) {
            if (cs.indexOf('aper') === -1) {
              return el.innerHTML = (cs === 'an article' ? 'a paper' : cs === 'article' ? 'paper' : 'Paper');
            }
          } else if (cs.indexOf('aper') !== -1) {
        ...
        if (this.config.pilot) {
          ...
          if (this.plugin === 'instantill') {
            ...
            pilot += '<a class="_oab_ping" message="shareyourpaper_use_the_old_form" target="_blank" href="' + (this.config.old_way ? (this.config.old_way.indexOf('@') !== -1 ? 'mailto:' : '') + this.config.old_way : 'mailto:' + this.cml()) + '">use the old way</a>.</p>';
          }
        ...
        if (this.plugin === 'shareyourpaper') {
          if (((this.cml() != null) || (this.config.old_way && this.config.old_way.includes('@'))) && (el = _L.gebi('_oab_nodoi'))) {
          ...
          if (this.config.not_library) {
            ...
            _L.html('#_oab_lib_info', 'Share your paper with help from the library in ' + ((ref3 = this.config.repo_name) != null ? ref3 : 'ScholarWorks') + '. Legally, for free, in minutes. ');
          }
          if (this.config.repo_name) {
        ...
        _L.listen('enter', '#_oab_input', (e) => {
          return this.find(e);
        });
        _L.listen('enter', '#_oab_email', (e) => {
          return this.validate(e);
        });
        _L.listen('click', '._oab_find', (e) => {
          return this.find(e);
        });
        _L.listen('click', '._oab_submit', (e) => {
          return this.submit(e);
        });
        _L.listen('click', '._oab_restart', (e) => {
          return this.restart(e);
        });
        _L.listen('click', '._oab_ping', (e) => {
          return this.ping(_L.get(e.target, 'message'));
        });
        ...
        _L.listen('click', '._oab_confirm', (e) => {
          e.preventDefault();
          this.data.confirmed = true;
          return this.deposit();
        });
        _L.listen('click', '#_oab_reviewemail', (e) => {
          return this.done('review');
        });
        _L.listen('click', '._oab_deposit', (e) => {
      ...
      if (window.location.search.indexOf('panel=') !== -1) {
        this.panel(window.location.search.split('panel=')[1].split('&')[0].split('#')[0], (window.location.search.indexOf('section=') !== -1 ? window.location.search.split('section=')[1].split('&')[0].split('#')[0] : void 0));
      }
      if (preview) {
